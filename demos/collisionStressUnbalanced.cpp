...
const int CONTAINER_THICKNESS = 20;
const int CONTAINER_DIMENSION = 150;
...
// Container of lots of colliders
void addContainer() {
	Vector dimensions = {CONTAINER_THICKNESS, CONTAINER_DIMENSION};

	Entity* lWall = mainScene->createEntity();
	dimensions = {CONTAINER_THICKNESS, CONTAINER_DIMENSION + CONTAINER_THICKNESS};
	lWall->addComponent<BoxCollider>(dimensions.x, dimensions.y);
	lWall->addComponent<RectangleRenderer>(dimensions.x, dimensions.y, Color(34, 102, 102, 102));
	lWall->phys->is_static = true;
	lWall->phys->mass = 100000;
	lWall->transform->position.x -= CONTAINER_DIMENSION/2;

	Entity* rWall = mainScene->createEntity();
	dimensions = {CONTAINER_THICKNESS, CONTAINER_DIMENSION + CONTAINER_THICKNESS};
	rWall->addComponent<BoxCollider>(dimensions.x, dimensions.y);
	rWall->addComponent<RectangleRenderer>(dimensions.x, dimensions.y, Color(34, 102, 102, 102));
	rWall->phys->is_static = true;
	rWall->phys->mass = 100000;
	rWall->transform->position.x += CONTAINER_DIMENSION/2;

	Entity* bWall = mainScene->createEntity();
	dimensions = {CONTAINER_DIMENSION + CONTAINER_THICKNESS, CONTAINER_THICKNESS};
	bWall->addComponent<BoxCollider>(dimensions.x, dimensions.y);
	bWall->addComponent<RectangleRenderer>(dimensions.x, dimensions.y, Color(34, 102, 102, 102));
	bWall->phys->is_static = true;
	bWall->phys->mass = 100000;
	bWall->transform->position.y -= CONTAINER_DIMENSION/2;

	Entity* tWall = mainScene->createEntity();
	dimensions = {CONTAINER_DIMENSION + CONTAINER_THICKNESS, CONTAINER_THICKNESS};
	tWall->addComponent<BoxCollider>(dimensions.x, dimensions.y);
	tWall->addComponent<RectangleRenderer>(dimensions.x, dimensions.y, Color(34, 102, 102, 102));
	tWall->phys->is_static = true;
	tWall->phys->mass = 100000;
	tWall->transform->position.y += CONTAINER_DIMENSION/2;

	for(int i = 0; i < NUM_CONTAINER_BALLS; ++i) {
		double abs = CONTAINER_DIMENSION/2; 
		Entity* newBall = createBall(generateRandomDouble(-abs, abs), generateRandomDouble(-abs, abs));
	}	
}
...
int main() {
	try
	{
		...
		createFramesCounter();
		createTimeCounter();
		createObjCounter(numSpawned + NUM_CONTAINER_BALLS);
		addContainer();
